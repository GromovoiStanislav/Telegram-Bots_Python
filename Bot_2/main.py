...
def open_website(message):
    ...
    markup.add(types.InlineKeyboardButton("Перейти на сайт", url="https://udemy.com"))
...
def mess(message):
    ...
    if get_message_bot == "начать тест заново":
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("Посмотреть курсы по Unity", url="https://www.udemy.com/courses/search/?src=ukw&q=unity"))
        final_message = "Для разработки игр под мобильные устройства зачастую используется игровой движок <a href='https://unity.com/ru'>Unity</a>\nДвижок прост в изучении и вы можете просмотреть курсы по нему по кнопке ниже"
